...
%
% Future enhancements:
%    1. enable interactive desktop rbbox that does not necesarily start within a figure
%    2. enable screen-capture to the system clipboard
%
% See also:
...
% $Revision: 1.3 $  $Date: 2012/07/29 09:43:35 $
    ...
    paramsStruct = convertPos(paramsStruct);
    ...
    if ~isempty(paramsStruct.filename)
        if ~isempty(imgData)
            ...
            msgbox(['No image area selected - not saving image file ' paramsStruct.filename],'ScreenCapture','warn');
        end
    end
    ...
    elseif isempty(paramsStruct.filename) & ~isempty(imgData)  %#ok ML6
...
function paramsStruct = processArgs(varargin)
    ...
    try
        ...
        paramsStruct.wasDocked = false;
        paramsStruct.wasInteractive = false;
...
%% Convert position from handle-relative to desktop Java-based pixels
function paramsStruct = convertPos(paramsStruct)
    try
        ...
        try
            if strcmpi(get(paramsStruct.hFigure,'WindowStyle'),'docked')
                ...
                drawnow; pause(0.25);
                paramsStruct.wasDocked = true;
            end
        catch
            % never mind - ignore...
        end

        % The figure must be in focus
        ...
        if isempty(paramsStruct.handle)
            ...
            if isempty(paramsStruct.position)
                paramsStruct.position = getInteractivePosition;
                paramsStruct.wasInteractive = true;
                wasPositionGiven = false;
            end
...
function positionRect = getInteractivePosition
    uiwait(msgbox({'Mouse-click in any Matlab figure and then', ...
                   'drag a bounding rectangle for screen-capture,', ...
                   'or single-click to capture the entire figure'},'ScreenCapture'));
...
% - Enh: enable screen-capture to the clipboard