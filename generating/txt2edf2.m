function [duration, matrix_length] = txt2edf2(filename, desired_filename)
% txt2edf(filename, desired_filename);
% Converts a .txt file converted from .mcd in MC_DataTool to an
% .edf file that can be read by Neuroscore.
% desired_filename is optional.
...
if exist('edffid','var') % if we successfully opened the new file
    ...
    matrix = textscan(txtfid,format,1000000,'CollectOutput',1); %grabs the first 1000000 rows of the .txt document
    ...
    while ~isempty(matrix)
        ...
        matrix = textscan(txtfid,format,1000000,'CollectOutput',1); %grabs the next 1000000 rows of the .txt document
    ...
    % Header information.
    % All numbers must be in single quotes to properly construct the string
    header.version = '0';
