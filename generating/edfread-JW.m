...
% HEADER
hdr.version        = str2double(char(fread(fid,8)'));
...
hdr.length      = str2double(fread(fid,8,'*char')');
hdr.reserved44       = fread(fid,44,'*char')';
hdr.numofepochs    = str2double(fread(fid,8,'*char')');
hdr.epochduration   = str2double(fread(fid,8,'*char')');
% Number of signals
hdr.numofsignals    = str2double(fread(fid,4,'*char')');
for ii = 1:hdr.numofsignals
    hdr.label{ii} = fread(fid,16,'*char')';
end
for ii = 1:hdr.numofsignals
    hdr.transducertype{ii} = fread(fid,80,'*char')';
end
% Physical dimension
for ii = 1:hdr.numofsignals
    hdr.physdimension{ii} = fread(fid,8,'*char')';
end
% Physical minimum
for ii = 1:hdr.numofsignals
    hdr.physmin(ii) = str2double(fread(fid,8,'*char')');
end
% Physical maximum
for ii = 1:hdr.numofsignals
    hdr.physmax(ii) = str2double(fread(fid,8,'*char')');
end
% Digital minimum
for ii = 1:hdr.numofsignals
    hdr.digimin(ii) = str2double(fread(fid,8,'*char')');
end
% Digital maximum
for ii = 1:hdr.numofsignals
    hdr.digimax(ii) = str2double(fread(fid,8,'*char')');
end
for ii = 1:hdr.numofsignals
    hdr.prefilt{ii} = fread(fid,80,'*char')';
end
for ii = 1:hdr.numofsignals
    hdr.samplerate(ii) = str2double(fread(fid,8,'*char')');
end
for ii = 1:hdr.numofsignals
    hdr.reserved32    = fread(fid,32,'*char')';
end
hdr.label = deblank(hdr.label);
hdr.physdimension = deblank(hdr.physdimension);


if nargout > 1 || assignToVariables
    % Scale data (linear scaling)
%     scalefac = (hdr.physmax - hdr.physmin)./(hdr.digimax - hdr.digimin);
%     dc = hdr.physmax - scalefac .* hdr.digimax;
%     
    % RECORD DATA REQUESTED
    tmpdata = struct;
    for recnum = 1:hdr.numofepochs
        for ii = 1:hdr.numofsignals
            % Use a cell array for DATA because number of samples may vary
            % from sample to sample
            tmpdata(recnum).data{ii} = fread(fid,hdr.samplerate(ii),'int16');% * scalefac(ii) + dc(ii);
        end
    end
    record = zeros(hdr.samplerate(1)*hdr.numofepochs, hdr.numofsignals);
    
    for ii = 1:numel(hdr.label)
        ctr = 1;
        for jj = 1:hdr.numofepochs
            try
                record(ctr : ctr + hdr.samplerate - 1, ii) = tmpdata(jj).data{ii};
            end
            ctr = ctr + hdr.samplerate;
        end
    end
