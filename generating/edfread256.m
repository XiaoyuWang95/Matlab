...
%     
    ...
    record = zeros(256*hdr.duration*hdr.records, hdr.ns);

    for ii = 1:numel(hdr.label)
        ctr = 1;
        for jj = 1:hdr.records
            record(ctr : ctr + (256*hdr.duration) - 1, ii) = compress(tmpdata(jj).data{ii},hdr.samples(ii)/hdr.duration,256);
            ctr = ctr + (256*hdr.duration);
        end
    end
    ...
    for i=1:numel(hdr.samples)
        hdr.samples(i) = 256*hdr.duration;
    end
end
fclose(fid);